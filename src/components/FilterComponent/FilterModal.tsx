...
const  FilterModal: React.FC<FilterModalComponentProps> = ({closeModal,submitFilterData, clearAllFilter}:FilterModalComponentProps) => {
    ...
    useEffect(() =>{
            const data = deepCloneCustom([...transformDataForFilterModalUI(getAllData)]);
            if(data){
                let tabIdArr:any = [];
    ...
    return <>
        <div className="overflow-auto md:h-112.5 md:max-h-127.5 bg-white fixed z-300 left-0 right-0 bottom-0 md:absolute md:top-[50%] md:left-[50%] md:transform md:-translate-x-1/2 md:-translate-y-1/2 rounded-t-2xl md:rounded-2xl">
            <div className="flex flex-row justify-between items-center p-2 text-sm font-bold border-b-2 border-b-slate-400"><h1>Filters</h1><X className={'text-slate-950 cursor-pointer'} onClick={handleCloseModal} width={16} height={20}/></div>
            <div className="flex max-h-87.5 h-fit">
                <div className="flex flex-col h-fit md:h-87.5 max-h-87.5 overflow-auto overflow-x-auto">
                {tabID && tabID.map((item:any,index:number) => {
                    const filterKey = item as keyof typeof CORRESPONDING_FILTER_TABLE_KEY_NAME;
                    
                    return (
                        <div  tabIndex={index} onClick={handleTabs} key={index + 9*index} className={`flex-1 cursor-pointer text-xs basis-28 w-28 wrap-anywhere flex items-center justify-center  ${focusedIndex === index ? 'bg-white text-indigo-700 dark:text-indigo-300 dark:bg-slate-900 border-l-4 border-r-0 border-indigo-600 dark:border-indigo-400 font-semibold' : 'border-r-2 border-slate-400 text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700'} ${(index === 0) ? 'border-t-0' : (focusedIndex === index) ? 'border-t-2 border-t-slate-400' : 'border-t-2' }`}><h1 className="text-center">{CORRESPONDING_FILTER_TABLE_KEY_NAME[filterKey]}</h1></div>
                    )
                })}
                </div>
                <div className="flex-1">
                {tabValue && tabValue.map((item:any,index:number) => 
                    <React.Fragment key={index+2*item.length}>
                    {focusedIndex === index && 
                        <div  className="transition-all duration-200 ease-out outline-none bg-white p-2 relative flex flex-wrap items-center overflow-auto overflow-x-auto   max-h-87.5 h-fit md:h-87.5" itemID={`${index}`}>
                            {item[1]?.map((chipObj:any, chipId: number) => (
                                <React.Fragment key={chipId + 12*chipId}>
                                {chipId < 10 ? 
                             ...
                             {seeMore && <button onClick={handleSeeMore}>See More</button>}
                        </div>
                      }
                    </React.Fragment>
                )}
            ...
            <div className="flex bg-white justify-around flex-row p-2 border-t-2 border-t-slate-400">
                <button onClick={clearFilter} className="text-indigo-600 font-semibold text-sm cursor-pointer">Clear All</button>
